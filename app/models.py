...
- id is the natural id from the source data (string), not an autoincrement PK.
  The sample dataset already supplies stable identifiers ("a1", "a2"...).
  Re-using the source id is what makes idempotent import possible: on
  re-import we look up by this id rather than by guessing equality on
  other fields.
- tags stored as JSON array, metadata as JSON blob. Postgres JSONB would be
  the production choice (indexable, queryable) — using JSON here is a
  stated scope assumption (see README).
